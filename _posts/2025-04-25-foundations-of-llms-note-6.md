...
LongLLMLingua 框架**利用小模型评估 Token 的困惑度，然后基于困惑度删除冗余文本**。其首先进行问题感知的粗粒度压缩，即在给定问题条件下通过计算文档中所有Token 困惑度的均值来评估文档的重要性，困惑度越高表示文档信息量越大。随后，执行问题感知的细粒度压缩，即进一步计算文档中每个 Token 的困惑度并去除其中低困惑度的 Token。此外，论文还引入了**文档重排序机制**、**动态压缩比率**以及**子序列恢复机制**，确保重要文档和文档中的重要信息被有效利用。
